...
if __name__ == '__main__':
    dataPath = r'F:\DataBase\datasetpro\defog\imagesfogpro\\'
    deResPath=r'F:\DataBase\datasetpro\defog\imagespro\\'
#循环体


#循环体结束
    for file_img in os.listdir(dataPath):
        tic = time.time()
        im=cv2.imread(dataPath + file_img)
        #<原函数
        m = deHaze((im / 255.0) )* 255
        #原函数/>
        #将浮点型阵列转化为uint8阵列
        m = m.astype(np.uint8)
        #<change 9
        ...
        #change9 />
        cv2.imwrite(deResPath+file_img, m)
        print('success')
